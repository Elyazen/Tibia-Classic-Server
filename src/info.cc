...
int GetWeight(Object Obj, int Count){
	...
	if(ObjType.getFlag(TAKE)){
		...
		error("GetWeight: Object type %d is not acceptable.\n", ObjType.TypeID);
	}
	return Result;
}
...
bool ObjectInRange(uint32 CreatureID, Object Obj, int Range){
	...
	if(Creature == NULL){
		error("ObjectInRange: Invalid creature CreatureID=%d hand over.\n", CreatureID);
		return false;
	}
...
Object GetBodyContainer(uint32 CreatureID, int Position){
	...
	if(Position >= INVENTORY_FIRST && Position <= INVENTORY_LAST){
		if(!Creature->CrObject.exists()){
			error("GetBodyContainer: Creature object from %s does not exist (pos %d).\n",
...
void CalculateChange(int Amount, int *Gold, int *Platinum, int *Crystal){
	...
	print(3, "Pay %d with %d/%d/%d Coins...\n", Amount, Go, Pl, Cr);
...
bool SearchFlightField(uint32 FugitiveID, uint32 PursuerID, int *x, int *y, int *z){
	...
	if(Fugitive == NULL){
		error("SearchFlightField: Refugee does not exist.\n");
		return false;
	}

	TCreature *Pursuer = GetCreature(PursuerID);
	if(Pursuer == NULL){
		error("SearchFlightField: Tracker does not exist.\n");
		return false;
	}

	if(Fugitive->posz != Pursuer->posz){
		error("SearchFlightField: Refugee and persecutor are on different levels.\n");
		return false;
	}
