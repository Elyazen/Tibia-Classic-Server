...
bool TCreature::SetOnMap(void){
	...
	try{
		...
		error("TCreature::SetOnMap: Cannot set creature ([%d,%d,%d] - Exception %d).\n",
...
void TCreature::ToDoTrade(int ObjX, int ObjY, int ObjZ, ObjectType Type, uint8 RNum,
	...
	if(TradePartner == 0){
		error("TCreature::ToDoTrade: Trading partner is zero.\n");
		throw ERROR;
	}
	...
	if(Creature->Type != PLAYER){
		error("TCreature::ToDoTrade: Trading partner of %s is not a player.\n", this->Name);
		throw ERROR;
	}
...
void TCreature::NotifyGo(void){
	...
	if(this->Type == PLAYER){
		...
		if(TradeObject != NONE){
			...
			if(!TradeObject.exists()){
				error("TCreature::NotifyGo: Trading object no longer exists.\n");
				error("# Dealer %s to [%d,%d,%d]\n", this->Name, DestX, DestY, DestZ);
				if(Partner != NULL){
	...
	if(Bank != NONE){
		...
		error("TCreature::NotifyGo: On field [%d,%d,%d] there is no bank.\n",
				DestX, DestY, DestZ);
	}
}
