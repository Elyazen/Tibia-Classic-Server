...
void ShowSubownerList(uint16 HouseID, TPlayer *Player, char *Buffer){
	...
	print(3, "Edit the house's subtenant list %d.\n", HouseID);
...
void ShowGuestList(uint16 HouseID, TPlayer *Player, char *Buffer){
	...
	print(3, "Edit house guest list %d.\n", HouseID);
...
void ChangeSubowners(uint16 HouseID, TPlayer *Player, const char *Buffer){
	...
	Log("houses", "%s changes list of subtenants of house %d.\n", Player->Name, HouseID);
...
void ChangeGuests(uint16 HouseID, TPlayer *Player, const char *Buffer){
	...
	Log("houses", "%s changes list of house guests %d.\n", Player->Name, HouseID);
...
void KickGuest(uint16 HouseID, TPlayer *Guest){
	...
	if(Bed != NONE && Bed.getAttribute(TEXTSTRING) != 0){
		print(3, "Player stands on a bed while being kicked out of the house.\n");
		try{
			UseObjects(0, Bed, Bed);
		}catch(RESULT r){
			error("KickGuest: Exception %d when tidying up the bed.\n", r);
		}
	}
...
void ShowNameDoor(Object Door, TPlayer *Player, char *Buffer){
	...
	if(HouseID == 0){
		error("ShowNameDoor: Door on coordinate [%d,%d,%d] does not belong to any house.\n",
				DoorX, DoorY, DoorZ);
		throw ERROR;
	}

	if(!IsOwner(HouseID, Player)){
		print(3, "player %s is not a tenant of the house %d.\n",
				Player->Name, HouseID);
		throw NOTACCESSIBLE;
	}
	...
	if(!Door.getObjectType().getFlag(TEXT)){
		error("ShowNameDoor: Door on coordinate [%d,%d,%d] contains no text.\n",
				DoorX, DoorY, DoorZ);
		throw ERROR;
	}
...
void ChangeNameDoor(Object Door, TPlayer *Player, const char *Buffer){
	...
	uint16 HouseID = GetHouseID(DoorX, DoorY, DoorZ);
	if(HouseID == 0){
		error("ChangeNameDoor: Door on coordinate [%d,%d,%d] does not belong to any house.\n",
				DoorX, DoorY, DoorZ);
		throw ERROR;
	}
	...
	// TODO(fusion): Check for `NAMEDOOR` flag as well?
	if(!Door.getObjectType().getFlag(TEXT)){
		error("ChangeNameDoor: Door on coordinate [%d,%d,%d] contains no text.\n",
				DoorX, DoorY, DoorZ);
		throw ERROR;
	}

	Log("houses", "%s changes NameDoor from home %d on coordinate [%d,%d,%d].\n",
...
bool FinishAuctions(void){
	...
	for(int AuctionNr = 0; AuctionNr < NumberOfAuctions; AuctionNr += 1){
		...
		Log("houses", "Auction for house %d finished: Auctioned by %s for %d Gold.\n",
		...
		THouse *House = GetHouse(HouseID);
		if(House == NULL){
			error("FinishAuctions: House with a number %d does not exist.\n",
					HouseID);
			continue;
		}

		if(House->OwnerID != 0){
			error("FinishAuctions: House with a number %d is already in character %u.\n",
		...
		if(DepotMoney < (House->Rent + Bid)){
			Log("houses", "The winning bidder does not have enough money.\n");
			House->OwnerID = CharacterID;
			strcpy(House->OwnerName, CharacterName);
			House->PaidUntil = 0;
		}else{
			Log("houses", "The winning bidder has enough money.\n");
	...
	for(int HouseNr = 0; HouseNr < Houses; HouseNr += 1){
		...
		if(H->PaidUntil == 0 && H->OwnerID != 0){
			if(QueryManagerConnection->excludeFromAuctions(H->OwnerID, true) != 0){
				error("FinishAuctions: Cannot ban bidders.\n");
			}
...
bool TransferHouses(void){
	Log("houses", "Process voluntary redundancies...\n");
	...
	for(int TransferNr = 0; TransferNr < NumberOfTransfers; TransferNr += 1){
		...
		THouse *House = GetHouse(HouseID);
		if(House == NULL){
			error("CollectRents: House with a number %d does not exist.\n", HouseID);
			continue;
		}

		if(NewOwnerID != 0 && TransferPrice > 0){
			Log("houses", "Sell ​​house %d to %u for %d Gold.\n",
			...
			if(DepotMoney < TransferPrice){
				Log("houses", "Buyer doesn't have enough money.\n");
		...
		// to a new owner?
		Log("houses", "Rooms house %d from %u.\n", HouseID, House->OwnerID);
		if(NewOwnerID == 0){
		...
		if(NewOwnerID != 0){
			Log("houses", "Transfer house %d to %u.\n", HouseID, NewOwnerID);
...
bool EvictFreeAccounts(void){
	...
	if(Ret != 0){
		error("CollectRents: Cannot determine payment details.\n");
		return false;
	}
	...
	for(int EvictionNr = 0; EvictionNr < NumberOfEvictions; EvictionNr += 1){
		...
		Log("houses", "Account from house %d is no longer paid.\n", HouseID);
		if(OwnerID != House->OwnerID){
			Log("houses", "... but the house was just transferred.\n");
			continue;
		}
...
bool EvictExGuildLeaders(void){
	...
	if(NumberOfGuildHouses > 0){
		...
		for(int EvictionNr = 0; EvictionNr < NumberOfEvictions; EvictionNr += 1){
			...
			Log("houses", "Tenants of Guild House %d is no longer a guild leader.\n", HouseID);
			CleanHouse(House, NULL);
			ClearHouse(House);
			House->Help = 1;
		}
...
void CollectRent(void){
	...
	for(int HouseNr = 0; HouseNr < Houses; HouseNr += 1){
		...
		Log("houses", "Collect rent for house %d from %u a.\n", H->ID, H->OwnerID);
		...
		if(DepotMoney < H->Rent){
			if(TimeStamp < Deadline){
				Log("houses", "Tenant receives reminder.\n");
				...
				Log("houses", "Tenant is evicted.\n");

				if(QueryManagerConnection->excludeFromAuctions(H->OwnerID, false) != 0){
					error("CollectRents: Cannot ban tenants.\n");
				}
	...
	for(int HouseNr = 0; HouseNr < Houses; HouseNr += 1){
		...
		if(H->Help == 1){
			if(QueryManagerConnection->deleteHouseOwner(H->ID) != 0){
				error("CollectRents: Can house %d do not remove from HouseOwners.\n", H->ID);
			}
		}
	}
...
bool UpdateHouseOwners(void){
	...
	Log("houses", "Update list of tenants...\n");
	for(int HouseNr = 0; HouseNr < Houses; HouseNr += 1){
		THouse *H = House.at(HouseNr);

		if(H->OwnerID == 0 && H->Help != 0){
			Log("houses", "No longer rented house %d out of.\n", H->ID);
			if(QueryManagerConnection->deleteHouseOwner(H->ID) != 0){
				error("StartAuctions: Can no longer rent house %d not discharge.\n", H->ID);
			}
		}

		if(H->OwnerID != 0 && H->Help == 0){
			Log("houses", "Wear rented house %d a.\n", H->ID);
			if(QueryManagerConnection->insertHouseOwner(H->ID, H->OwnerID, H->PaidUntil) != 0){
				error("StartAuctions: Can rent house %d do not enter.\n", H->ID);
			}
		}

		if(H->Help == 2){
			Log("houses", "Update rented house %d.\n", H->ID);
			if(QueryManagerConnection->updateHouseOwner(H->ID, H->OwnerID, H->PaidUntil) != 0){
				error("StartAuctions: Can provide data of the rented house %d don't update.\n", H->ID);
			}
		}
	}
...
void CleanHouseField(int x, int y, int z){
	...
	if(HouseID == 0){
		error("CleanHouseField: No house in the field [%d,%d,%d] found.\n", x, y, z);
		return;
	}
...
void LoadHouseAreas(void){
	Log("houses", "Loading data of the house areas...\n");

	char FileName[4096];
	snprintf(FileName, sizeof(FileName), "%s/houseareas.dat", DATAPATH);
	if(!FileExists(FileName)){
		Log("houses", "No data found for house areas.\n");
		return;
	}
...
void LoadHouses(void){
	...
	while(true){
		...
		if(Area == NULL){
			error("LoadHouses: territory by house %d does not exist.\n", HouseID);
			throw "cannot load houses";
		}
		...
		if(H->ExitX == 0){
			error("LoadHouses: Output for house %d not set.\n", HouseID);
		}
	...
	if(Ret != 0){
		error("LoadHouses: Cannot enter house master data.\n");
	}
}
...
void LoadOwners(void){
	...
	if(!FileExists(FileName)){
		Log("houses", "No tenant details found.\n");
		return;
	}
	...
	while(true){
		...
		if(strcmp(Identifier, "extension") == 0){
			...
			THouse *House = GetHouse(HouseID);
			if(House == NULL){
				error("LoadOwners: House to ID %d does not exist.", HouseID);
	...
	int Ret = QueryManagerConnection->getHouseOwners(&NumberOfOwners,
						HouseIDs, OwnerIDs, OwnerNames, PaidUntils);
	if(Ret != 0){
		error("LoadOwners: Cannot get tenant names.\n");
		throw "Cannot load owners";
	}
			...
			for(int FieldX = MinX; FieldX <= MaxX; FieldX += 1){
				...
				if(Bed != NONE && Bed.getAttribute(TEXTSTRING) != 0){
					try{
						...
						error("LoadOwners: Exception %d when tidying up a bed.\n", r);
					}
				}
			}
		}
