...
TDrunkenImpact::TDrunkenImpact(TCreature *Actor, int Power, int Duration){
	...
	if(Power > 6){
		error("TDrunkenImpact::TDrunkenImpact: Power is too big (%d).\n", Power);
		Power = 6;
	}
...
void ActorShapeSpell(TCreature *Actor, TImpact *Impact, int Effect){
	if(Actor == NULL){
		error("ActorShapeSpell: Actor does not exist.\n");
		return;
	}
...
void CheckAccount(TCreature *Actor, int SpellNr){
	...
	if(SpellNr < 1 || SpellNr >= NARRAY(SpellList)){
		error("CheckAccount: Invalid check number %d.\n", SpellNr);
		throw ERROR;
	}
...
void Combat(TCreature *Actor, Object Target, int ManaPoints, int SoulPoints,
	...
	if(Actor == NULL){
		error("Combat: Surrendered creature does not exist.\n");
		throw ERROR;
	}
...
void CreateField(TCreature *Actor, Object Target, int ManaPoints, int SoulPoints, int FieldType){
	if(Actor == NULL){
		error("CreateField: Passed invalid creature.\n");
		throw ERROR;
	}
...
void MassCreateField(TCreature *Actor, Object Target,
	...
	if(Actor == NULL){
		error("MassCreateField: Passed invalid creature.\n");
		throw ERROR;
	}
...
void CreateFieldWall(TCreature *Actor, Object Target,
	...
	if(Actor == NULL){
		error("CreateFieldWall: Passed invalid creature.\n");
		throw ERROR;
	}
...
void DeleteField(TCreature *Actor, Object Target, int ManaPoints, int SoulPoints){
	if(Actor == NULL){
		error("DeleteField: Pass invalid creature.\n");
		throw ERROR;
	}
...
void CleanupField(TCreature *Actor, Object Target, int ManaPoints, int SoulPoints){
	if(Actor == NULL){
		error("CleanupField: Passed invalid creature.\n");
		throw ERROR;
	}
...
void Teleport(TCreature *Actor, const char *Param){
	if(Actor == NULL){
		error("Teleport: Pass invalid creature.\n");
		throw ERROR;
	}

	if(Param == NULL){
		error("Teleport: Param is ZERO.\n");
		throw ERROR;
	}
...
void TeleportToCreature(TCreature *Actor, const char *Name){
	if(Actor == NULL){
		error("TeleportToCreature: Pass invalid creature.\n");
		throw ERROR;
	}
...
void TeleportPlayerToMe(TCreature *Actor, const char *Name){
	if(Actor == NULL){
		error("TeleportPlayerToMe: Passed invalid creature.\n");
		throw ERROR;
	}
...
void MagicRope(TCreature *Actor, int ManaPoints, int SoulPoints){
	if(Actor == NULL){
		error("MagicRope: Pass invalid creature.\n");
		throw ERROR;
	}
...
void MagicClimbing(TCreature *Actor, int ManaPoints, int SoulPoints, const char *Param){
	if(Actor == NULL){
		error("MagicClimbing: Pass invalid creature.\n");
		throw ERROR;
	}
...
void CreateThing(TCreature *Actor, const char *Param1, const char *Param2){
	if(Actor == NULL){
		error("CreateThing: Passed invalid creature.\n");
		throw ERROR;
	}
...
void CreateMoney(TCreature *Actor, const char *Param){
	...
	if(Actor->Type != PLAYER){
		error("CreateMoney: Spell can only be used by player.\n");
		throw ERROR;
	}
...
void CreateFood(TCreature *Actor, int ManaPoints, int SoulPoints){
	if(Actor == NULL){
		error("CreateFood: Passed invalid creature.\n");
		throw ERROR;
	}
...
void CreateArrows(TCreature *Actor, int ManaPoints, int SoulPoints, int ArrowType, int Count){
	if(Actor == NULL){
		error("CreateArrows: Passed invalid creature.\n");
		throw ERROR;
	}
...
void SummonCreature(TCreature *Actor, int ManaPoints, int Race, bool God){
	if(Actor == NULL){
		error("SummonCreature: Pass invalid creature.\n");
		throw ERROR;
	}

	if(!IsRaceValid(Race)){
		error("SummonCreature: Invalid race number %d hand over.\n", Race);
		throw ERROR;
	}
...
void StartMonsterraid(TCreature *Actor, const char *RaidName){
	if(Actor == NULL){
		error("StartMonsterraid: Pass invalid creature.\n");
		throw ERROR;
	}
...
void RaiseDead(TCreature *Actor, Object Target, int ManaPoints, int SoulPoints){
	if(Actor == NULL){
		error("RaiseDead: Passed invalid creature.\n");
		throw ERROR;
	}
...
void MassRaiseDead(TCreature *Actor, Object Target, int ManaPoints, int SoulPoints, int Radius){
	if(Actor == NULL){
		error("MassRaiseDead: Pass invalid creature.\n");
		throw ERROR;
	}
...
void Heal(TCreature *Actor, int ManaPoints, int SoulPoints, int Amount){
	if(Actor == NULL){
		error("Heal: Pass invalid creature.\n");
		throw ERROR;
	}
...
void MassHeal(TCreature *Actor, Object Target, int ManaPoints, int SoulPoints, int Amount, int Radius){
	if(Actor == NULL){
		error("MassHeal: Pass invalid creature.\n");
		throw ERROR;
	}
...
void HealFriend(TCreature *Actor, const char *TargetName, int ManaPoints, int SoulPoints, int Amount){
	if(Actor == NULL){
		error("HealFriend: Pass invalid creature.\n");
		throw ERROR;
	}

	if(TargetName == NULL){
		error("HealFriend: Passed invalid name.\n");
		throw ERROR;
	}
...
void RefreshMana(TCreature *Actor, int ManaPoints, int SoulPoints, int Amount){
	if(Actor == NULL){
		error("RefreshMana: Pass invalid creature.\n");
		throw ERROR;
	}
...
void MagicGoStrength(TCreature *Actor, TCreature *Target, int ManaPoints, int SoulPoints, int Percent, int Duration){
	if(Actor == NULL){
		error("MagicGoStrength: Passed creature doesn't exist.\n");
		throw ERROR;
	}
...
void Shielding(TCreature *Actor, int ManaPoints, int SoulPoints, int Duration){
	if(Actor == NULL){
		error("Shielding: Pass invalid creature.\n");
		throw ERROR;
	}
...
void NegatePoison(TCreature *Actor, TCreature *Target, int ManaPoints, int SoulPoints){
	if(Actor == NULL){
		error("NegatePoison: Pass invalid creature.\n");
		throw ERROR;
	}
...
void Enlight(TCreature *Actor, int ManaPoints, int SoulPoints, int Radius, int Duration){
	if(Actor == NULL){
		error("Enlight: Pass invalid creature.\n");
		throw ERROR;
	}
...
void Invisibility(TCreature *Actor, int ManaPoints, int SoulPoints, int Duration){
	if(Actor == NULL){
		error("Invisibility: Pass invalid creature.\n");
		throw ERROR;
	}
...
void CancelInvisibility(TCreature *Actor, Object Target, int ManaPoints, int SoulPoints, int Radius){
	if(Actor == NULL){
		error("CancelInvisibility: Pass invalid creature.\n");
		throw ERROR;
	}
...
void CreatureIllusion(TCreature *Actor, int ManaPoints, int SoulPoints, const char *RaceName, int Duration){
	if(Actor == NULL){
		error("CreatureIllusion: Pass invalid creature.\n");
		throw ERROR;
	}

	if(RaceName == NULL){
		error("CreatureIllusion: Invalid racial name passed.\n");
		throw ERROR;
	}
...
void ObjectIllusion(TCreature *Actor, int ManaPoints, int SoulPoints, Object Target, int Duration){
	if(Actor == NULL){
		error("ObjectIllusion: Pass invalid creature.\n");
		throw ERROR;
	}
...
void ChangeData(TCreature *Actor, const char *Param){
	if(Actor == NULL){
		error("ChangeData: Passed invalid creature.\n");
		throw ERROR;
	}
...
void EnchantObject(TCreature *Actor, int ManaPoints, int SoulPoints, ObjectType OldType, ObjectType NewType){
	if(Actor == NULL){
		error("EnchantObject: Passed invalid creature.\n");
		throw ERROR;
	}
...
void FindPerson(TCreature *Actor, int ManaPoints, int SoulPoints, const char *TargetName){
	...
	if(Actor == NULL){
		error("FindPerson: Pass invalid creature.\n");
		throw ERROR;
	}
...
void GetPosition(TCreature *Actor){
	if(Actor == NULL){
		error("GetPosition: Pass invalid creature.\n");
		throw ERROR;
	}
...
void GetQuestValue(TCreature *Actor, const char *Param){
	if(Actor == NULL){
		error("GetQuestValue: Pass invalid creature.\n");
		throw ERROR;
	}
...
void SetQuestValue(TCreature *Actor, const char *Param1, const char *Param2){
	if(Actor == NULL){
		error("SetQuestValue: Pass invalid creature.\n");
		throw ERROR;
	}
...
void ClearQuestValues(TCreature *Actor){
	if(Actor == NULL){
		error("ClearQuestValues: Pass invalid creature.\n");
		throw ERROR;
	}
...
void CreateKnowledge(TCreature *Actor, const char *Param1, const char *Param2){
	if(Actor == NULL){
		error("CreateKnowledge: Passed invalid creature.\n");
		throw ERROR;
	}
...
void ChangeProfession(TCreature *Actor, const char *Param){
	if(Actor == NULL){
		error("ChangeProfession: Pass invalid creature.\n");
		throw ERROR;
	}

	if(Param == NULL){
		error("ChangeProfession: Passed invalid parameter.\n");
		throw ERROR;
	}
...
void EditGuests(TCreature *Actor){
	if(Actor == NULL){
		error("EditGuests: Passed invalid creature.\n");
		throw ERROR;
	}
...
void EditNameDoor(TCreature *Actor){
	if(Actor == NULL){
		error("EditNameDoor: Passed invalid creature.\n");
		throw ERROR;
	}
...
void KickGuest(TCreature *Actor, const char *GuestName){
	if(Actor == NULL){
		error("KickGuest(magic): Pass invalid creature.\n");
		throw ERROR;
	}

	if(GuestName == NULL){
		error("KickGuest(magic): Passed invalid guest.\n");
		throw ERROR;
	}
...
void Notation(TCreature *Actor, const char *Name, const char *Comment){
	if(Actor == NULL){
		error("Notation: Pass invalid creature.\n");
		throw ERROR;
	}
...
void NameLock(TCreature *Actor, const char *Name){
	if(Actor == NULL){
		error("NameLock: Pass invalid creature.\n");
		throw ERROR;
	}
...
void BanishAccount(TCreature *Actor, const char *Name, int Duration, const char *Reason){
	if(Actor == NULL){
		error("BanishAccount: Pass invalid creature.\n");
		throw ERROR;
	}
...
void DeleteAccount(TCreature *Actor, const char *Name, const char *Reason){
	if(Actor == NULL){
		error("DeleteAccount: Pass invalid creature.\n");
		throw ERROR;
	}
...
void BanishCharacter(TCreature *Actor, const char *Name, int Duration, const char *Reason){
	if(Actor == NULL){
		error("BanishCharacter: Pass invalid creature.\n");
		throw ERROR;
	}
...
void DeleteCharacter(TCreature *Actor, const char *Name, const char *Reason){
	if(Actor == NULL){
		error("DeleteCharacter: Passed invalid creature.\n");
		throw ERROR;
	}
...
void SetNameRule(TCreature *Actor, const char *Name){
	if(Actor == NULL){
		error("SetNameRule: Pass invalid creature.\n");
		throw ERROR;
	}
	...
	if(Actor->Type != PLAYER){
		error("SetNameRule: Spell can only be used by player.\n");
		throw ERROR;
	}
...
void KickPlayer(TCreature *Actor, const char *Name){
	if(Actor == NULL){
		error("KickPlayer: Pass invalid creature.\n");
		throw ERROR;
	}
...
void HomeTeleport(TCreature *Actor, const char *Name){
	if(Actor == NULL){
		error("HomeTeleport: Pass invalid creature.\n");
		throw ERROR;
	}
...
static void RuneSpell(uint32 CreatureID, int SpellNr){
	...
	if(RuneGr == 0){
		error("RuneSpell: Spell %d is a runic spell, but has no rune.\n", SpellNr);
		throw ERROR;
	}
...
void GetMagicItemDescription(Object Obj, char *SpellString, int *MagicLevel){
	...
	*MagicLevel = 0;
	...
	ObjectType ObjType = Obj.getObjectType();
	if(!ObjType.getFlag(RUNE)){
		error("GetMagicItemDescription: Passed item is not magical.\n");
		return;
	}
...
void UseMagicItem(uint32 CreatureID, Object Obj, Object Dest){
	...
	int SpellNr = FindSpell(Obj);
	if(SpellNr == 0){
		error("UseMagicItem: For object %d there is no saying.\n",
				Obj.getObjectType().TypeID);
		throw ERROR;
	}
...
static void InitCircles(void){
	...
	if(IN.fail()){
		error("InitCircles: Can file %s don't open.\n", FileName);
		...
		for(int SyllableNr = 0;
			...
			if(strcmp(SpellSyllable[SyllableNr], Syllable) == 0){
				...
				if(SyllableCount > NARRAY(Spell->Syllable)){
					error("CreateSpell: Number of syllables exceeded in Spell %d.\n", SpellNr);
