...
bool CheckVisibility(int Command, TConnection *Connection, int x, int y, int z){
	...
	if(!Result){
		...
		print(3, "CheckVisibility: Field [%d,%d,%d] is from [%d,%d,%d]"
				" off not visible on command %d.\n",
...
bool CheckObjectType(int Command, int TypeID){
	...
	if(!Result){
		print(3, "CheckObjectType: Invalid object type %d at command %d.\n",
				TypeID, Command);
	}
	return Result;
}
...
void CSetTactics(TConnection *Connection, TReadBuffer *Buffer){
	...
	if(ChaseMode != CHASE_MODE_NONE
		...
		print(3, "CSetTactics: Invalid pursuit mode %d.\n", ChaseMode);
		return;
	}
...
void CSetOutfit(TConnection *Connection, TReadBuffer *Buffer){
	...
	if(NewOutfit.OutfitID < FirstOutfit || NewOutfit.OutfitID > LastOutfit){
		if(Player->Sex == 1){
			print(3, "CSetOutfit: Invalid outfit %d for men.\n", NewOutfit.OutfitID);
		}else{
			print(3, "CSetOutfit: Invalid outfit %d for women.\n", NewOutfit.OutfitID);
		}
		return;
	}
...
void ReceiveData(TConnection *Connection){
	...
	if((Connection->InDataSize + 2) > (int)sizeof(Connection->InData)){
		error("ReceiveData: Packet length too long %d.\n", Connection->InDataSize);
		return;
	}
	...
	try{
		...
		error("ReceiveData: Error reading the command (%s).\n", str);
		return;
	}
