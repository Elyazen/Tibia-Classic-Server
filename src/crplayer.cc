		...
		FormerAttackedPlayers(0, 10, 10)
{
	...
	if(!CheckRight(CharacterID, PREMIUM_ACCOUNT)){
		if(IsPremiumArea(this->startx, this->starty, this->startz)){
			Log("game", "player %s is expatriated from PayArea city and given a new hometown.\n", this->Name);
		...
		if(IsPremiumArea(this->posx, this->posy, this->posz)){
			Log("game", "player %s is kicked out of PayArea and placed in his hometown.\n", this->Name);
			this->posx = this->startx;
			this->posy = this->starty;
			this->posz = this->startz;
		}
	}else{
		Log("game", "Player has a premium account.\n");
	}
	...
	if(PlayerData->LastLoginTime != 0){
		...
		Log("game", "player %s logs in for the first time -> choose outfit.\n", this->Name);
		SendMessage(Connection, TALK_LOGIN_MESSAGE,
...
TPlayer::~TPlayer(void){
	...
	if(!this->IsDead){
		...
		if(this->Profession != PROFESSION_NONE && this->Skills[SKILL_LEVEL]->Get() <= 5){
			Log("game", "Set players %s completely back because of level.\n", this->Name);
			ResetCharacter = true;
		}else if(this->Skills[SKILL_HITPOINTS]->Max <= 0){
			Log("game", "Set players %s completely back because of MaxHitpoints.\n", this->Name);
			ResetCharacter = true;
		}
...
void TPlayer::Death(void){
	if(CheckRight(this->ID, INVULNERABLE)){
		error("TPlayer::Death: Aha, it doesn't work like that!! You can't kill gods!!\n");
		this->Skills[SKILL_HITPOINTS]->SetMax();
		return;
	}
...
void TPlayer::LoadInventory(bool SetStandardInventory){
	...
	if(PlayerData->Inventory != NULL){
		try{
			...
			error("TPlayer::LoadInventory: Can inventory from player %s don't read.\n", this->Name);
			error("# Mistake: %s\n", str);
		}
...
void TPlayer::SaveInventory(void){
	...
	try{
		...
		error("TPlayer::SaveInventory: Can save inventory from player %s don't write.\n", this->Name);
		error("# Mistake: %s\n", str);
	}
}
...
void TPlayer::TakeOver(TConnection *Connection){
	...
	if(PlayerData->Locked != gettid()){
		error("TPlayer::TakeOver: PlayerData slot is not locked correctly (%d).\n", PlayerData->Locked);
	}
...
void TPlayer::SetProfession(uint8 Profession){
	if(Profession == PROFESSION_PROMOTION){
		if(this->Profession == PROFESSION_NONE){
			error("TPlayer::SetProfession: Player does not yet have a profession for refining.\n");
			return;
		}

		if(this->Profession >= PROFESSION_PROMOTION){
			error("TPlayer::SetProfession: Player has already refined his profession.\n");
			return;
		}
...
void TPlayer::Regenerate(void){
	...
	if(!Bed.getObjectType().getFlag(TEXT)){
		error("TPlayer::Regenerate: Bed has no text.\n");
		return;
	}
	...
	try{
		...
		error("TPlayer::Regenerate: Exception %d when tidying up the bed.\n", r);
	}
}
...
void TPlayer::RecordAttack(uint32 VictimID){
	...
	if(!Victim->InPartyWith(this, true)
		...
		*this->AttackedPlayers.at(this->NumberOfAttackedPlayers) = VictimID;
		...
		print(3, "player %s is attacker for players %s.\n", this->Name, Victim->Name);
	...
	if(!this->IsAttackJustified(VictimID) && !this->Aggressor){
		this->Aggressor = true;
		print(3, "player %s is the aggressor.\n", this->Name);
...
void TPlayer::RecordMurder(uint32 VictimID){
	...
	if(Playerkilling != 0){
		...
		if(OldPlayerkillerEnd == 0){
			print(3, "player %s is player killer.\n", this->Name);
			AnnounceChangedCreature(this->ID, CREATURE_SKULL_CHANGED);
		}
...
void TPlayer::ClearPlayerkillingMarks(void){
	...
	if(this->Aggressor){
		...
		for(int i = 0; i < this->NumberOfFormerAttackedPlayers; i += 1){
			...
			if(Victim != NULL){
				print(3, "player %s is no longer an attacker for players %s.\n", this->Name, Victim->Name);
				if(Victim->Connection != NULL
				&& Victim->Connection->KnownCreature(this->ID, false) == KNOWNCREATURE_UPTODATE){
					SendCreatureSkull(Victim->Connection, this->ID);
				}
...
int TPlayer::GetPlayerkillingMark(TPlayer *Observer){
	if(WorldType == NORMAL){
		if(Observer == NULL){
			error("TPlayer::GetPlayerkillingMark: Watcher does not exist.\n");
			return SKULL_NONE;
		}
...
bool LoadPlayerData(TPlayerData *Slot){
	...
	try{
		...
		error("LoadPlayerData: Can load player items %u not loading.\n",
				Slot->CharacterID);
		error("# Mistake: %s\n", str);
	}catch(const std::bad_alloc &e){
		error("LoadPlayerData: No memory free when loading player %u.\n",
				Slot->CharacterID);
	}
...
void SavePlayerData(TPlayerData *Slot){
	...
	try{
		...
		error("SavePlayerData: Can save player items %u don't write.\n", Slot->CharacterID);
		error("# Mistake: %s\n", str);
		unlink(FileName);
	}
}
...
TPlayerData *AssignPlayerPoolSlot(uint32 CharacterID, bool DontWait){
	...
	print(3, "Reserve slot for character %u.\n", CharacterID);
	...
	print(3, "Load data for players %u.\n", CharacterID);
...
TPlayerData *AttachPlayerPoolSlot(uint32 CharacterID, bool DontWait){
	...
	print(3, "Attach slot from player %u.\n", CharacterID);
...
void DecreasePlayerPoolSlotSticky(uint32 CharacterID){
	...
	if(Slot != NULL){
		...
		error("DecreasePlayerPoolSlotSticky: Slot by player %u not found.\n", CharacterID);
	}
	PlayerDataPoolMutex.up();
}
...
void ReleasePlayerPoolSlot(TPlayerData *Slot){
	...
	print(3, "Give slot from player %u free.\n", Slot->CharacterID);
	if(Slot->Locked == 0){
...
void InitPlayerIndex(void){
	...
	while(true){
		...
		if(Ret != 0){
			error("InitPlayerIndex: Cannot get player data.\n");
			break;
		}
