...
void TConnection::Connect(int Socket){
	if(this->State != CONNECTION_ASSIGNED){
		error("TConnection::Connect: Connection is not assigned to a thread.\n");
	}
...
bool TConnection::JoinGame(TReadBuffer *Buffer){
	...
	try{
		...
		error("TConnection::JoinGame: Error reading buffer (%s).\n", str);
	}
	...
	if(Player == NULL){
		...
		if(Player->IsDead){
			Log("game", "player %s just dying - failed to log in.\n", Player->Name);
			DecreasePlayerPoolSlotSticky(this->CharacterID);
			return false;
		}

		if(Player->LoggingOut && Player->LogoutPossible() == 0){
			Log("game", "player %s currently logging out - login failed.\n", Player->Name);
			DecreasePlayerPoolSlotSticky(this->CharacterID);
			return false;
		}
...
void TConnection::UnchainKnownCreature(uint32 ID){
	...
	if(Creature->FirstKnowingConnection == NULL){
		error("TUserCom::UnchainKnownCreature: Creature %u nobody knows.\n", ID);
		return;
	}
