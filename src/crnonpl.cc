	...
	switch(Node->Type){
		...
		case BEHAVIOUR_NODE_PARAMETER:{
			if(Node->Data != 1 && Node->Data != 2){
				...
				error("TBehaviourDatabase::evaluate: Number parameter %d not documented.\n", Node->Data);
			}else{
			...
			switch(Action->Type){
				case BEHAVIOUR_ACTION_REPLY:{
					...
					if(FormatNpcResponse(Response, sizeof(Response), Template, Npc, Interlocutor)){
						...
						Response[20] = 0;
						error("TBehaviourDatabase::react: Text from NPC %s will be too long (%s...).\n",
								Npc->Name, Response);
					}
					break;
				}
				...
				case BEHAVIOUR_ACTION_SET_SKILL:{
					...
					if(SkillNr == SKILL_HITPOINTS){
						...
						if(Interlocutor->Skills[SKILL_HITPOINTS]->Get() <= 0){
							error("TBehaviourDatabase::react: NPC %s kills players.\n", Npc->Name);
						}
					}else{
				...
				case BEHAVIOUR_ACTION_CHANGESTATE:{
					...
					if(!StartToDo){
						...
						if(Situation != ADDRESSQUEUE){
							...
							error("TBehaviourDatabase::react: NPC %s does not respond to greetings %s.\n",
									Npc->Name, Text);
						}
					}else{
						if(NewState == IDLE){
				...
				case BEHAVIOUR_ACTION_FUNCTION3:{
					...
					switch(FunctionNr){
						case 1:{
							print(3, "NPC teleports interlocutors to [%d,%d,%d].\n",
						...
						case 2:{
							print(3, "NPC sets starting coordinate for conversation partner to [%d,%d,%d].\n",
									Param1, Param2, Param3);
...
void LoadMonsterhomes(void){
	...
	for(int i = 0; i < Monsterhomes; i += 1){
		...
		if(MH->Timer > 0){
			error("LoadMonsterhomes: Timer is already running (breed %d to [%d,%d,%d]).\n",
					MH->Race, MH->x, MH->y, MH->z);
	...
	for(int i = 1; i <= Monsterhomes; i += 1){
		...
		if(MaxRadius >= 0){
			...
			if(SearchSpawnField(&SpawnX, &SpawnY, &SpawnZ, SpawnRadius, false)){
				...
				if(MH->Timer > 0){
					error("ProcessMonsterhomes: Timer is already running (breed %d to [%d,%d,%d]).\n",
		...
		QueuedAddresses(0, 9, 10)
{
	...
	this->SetInList();
	if(!this->SetOnMap()){
		print(1, "WARNING: May NPC\'%s\' do not set.\n", this->Name);
		return;
	}
...
void TNPC::Enqueue(uint32 InterlocutorID, const char *Text){
	if(InterlocutorID == 0){
		error("TNPC::Enqueue: Call partner is zero.\n");
		return;
	}
		...
		TNonplayer()
{
	...
	while(this->Master != 0){
		...
		error("TMonster::TMonster: Children are not allowed to create their own children.\n");
	...
	this->SetInList();
	if(!this->SetOnMap()){
		error("TMonster::TMonster: Cannot put monsters on the map.\n");
		return;
	}
				...
				for(int j = 0; j < Repeat; j += 1){
					...
					if(Item.getContainer().getObjectType().isMapContainer()){
						error("TMonster::TMonster: Object falls on the map."
								" CarryStrength for race %d increase.\n", Race);
						Delete(Item, -1);
					}
				}
			}
...
bool TMonster::MovePossible(int x, int y, int z, bool Execute, bool Jump){
	...
	if(this->Skills[SKILL_GO_STRENGTH]->Act < 0){
		error("TMonster::MovePossible: Monsters %s to [%d,%d,%d] must not move.\n",
	...
	error("TMonster::MovePossible: Infinite loop suspected for %s to [%d,%d,%d].\n",
			this->Name, x, y, z);
	return false;
}
...
void TMonster::IdleStimulus(void){
	...
	if(this->LifeEndRound != 0 && this->LifeEndRound <= RoundNr){
		print(3, "Lifetime for %s expired.\n", this->Name);
	...
	if(this->Master != 0){
		...
		if(Master == NULL){
			if(MasterIsPlayer){
				print(3, "creature %s loses her player master.\n", this->Name);
			}else{
				print(3, "creature %s loses her mother monster.\n", this->Name);
			}
			ShouldDespawn = true;
...
void ChallengeMonster(TCreature *Challenger, TCreature *Monster){
	...
	if(Monster == NULL){
		error("ChallengeMonster: Monster doesn't exist.\n");
		return;
	}
