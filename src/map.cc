...
uint32 Object::getAttribute(INSTANCEATTRIBUTE Attribute){
	...
	if(AttributeOffset == -1){
		error("Object::getAttribute: Flag for attribute %d for object type %d not set.\n",
				Attribute, ObjType.TypeID);
		return 0;
	}

	if(AttributeOffset < 0 || AttributeOffset >= NARRAY(TObject::Attributes)){
		error("Object::getAttribute: Invalid offset %d for attribute %d for object type %d.\n",
...
void Object::setAttribute(INSTANCEATTRIBUTE Attribute, uint32 Value){
	...
	int AttributeOffset = ObjType.getAttributeOffset(Attribute);
	if(AttributeOffset == -1){
		error("Object::setAttribute: Flag for attribute %d for object type %d not set.\n",
				Attribute, ObjType.TypeID);
		return;
	}

	if(AttributeOffset < 0 || AttributeOffset >= NARRAY(TObject::Attributes)){
		error("Object::setAttribute: Invalid offset %d for attribute %d for object type %d.\n",
				AttributeOffset, Attribute, ObjType.TypeID);
		return;
	}
...
static void ResizeHashTable(void){
	...
	error("INFO: Hash table too small. Size is doubled up %d.\n", NewSize);
	...
	for(uint32 i = 1; i < OldSize; i += 1){
		if(HashTableType[i] != STATUS_FREE){
			...
			if(HashTableType[i] == STATUS_LOADED){
				...
				error("ResizeHashTable: Error reorganizing HashTable.\n");
			}
		}
	}
...
void SwapSector(void){
	...
	try{
		...
		error("FATAL ERROR in SwapSector: Cannot file \"%s\" do not create.\n", FileName);
		error("# Mistake: %s\n", str);
		abort();
	}
}
...
void UnswapSector(uintptr FileNumber){
	...
	try{
		...
		if(LoadingSector->Status != STATUS_SWAPPED){
			error("UnswapSector: Sector %d/%d/%d is not outsourced.\n", SectorX, SectorY, SectorZ);
			File.close();
			return;
		}
		...
		error("FATAL ERROR in UnswapSector: Cannot file \"%s\" don't read.\n", FileName);
		error("# Mistake: %s\n", str);
		abort();
	}
}
...
void LoadSector(const char *FileName, int SectorX, int SectorY, int SectorZ){
	...
	try{
		...
		error("LoadSector: Can file \"%s\" don't read.\n", FileName);
		error("# Mistake: %s\n", str);
...
void SaveSector(char *FileName, int SectorX, int SectorY, int SectorZ){
	...
	try{
		...
		error("SaveSector: Can file %s don't write.\n", FileName);
		error("# Mistake: %s\n", str);
	}
}
...
void RefreshSector(int SectorX, int SectorY, int SectorZ, TReadStream *Stream){
	...
	try{
		...
		error("RefreshSector: Error reading out the data (%s).\n", str);
	}
}
...
void PatchSector(int SectorX, int SectorY, int SectorZ, bool FullSector,
	...
	if(rename(FileNameBak, FileName) != 0){
		int ErrCode = errno;
		error("PatchSector: Error %d when renaming %s.\n", ErrCode, FileNameBak);
...
Object SplitObject(Object Obj, int Count){
	...
	if(!ObjType.getFlag(CUMULATIVE)){
		error("SplitObject: Object cannot be cumulated.\n");
...
void MergeObjects(Object Obj, Object Dest){
	...
	if(!ObjType.getFlag(CUMULATIVE)){
		error("MergeObjects: object type %d cannot be cumulated.\n", ObjType.TypeID);
		return;
	}
...
Object GetContainerObject(Object Con, int Index){
	if(Index < 0){
		error("GetContainerObject: Invalid serial number %d.\n", Index);
		return NONE;
	}
