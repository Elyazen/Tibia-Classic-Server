...
TCreature::~TCreature(void){
	...
	if(this->IsDead){
		...
		if(PoolLiquid != LIQUID_NONE){
			try{
				...
				if(r != NOROOM && r != DESTROYED){
					error("TCreature::~TCreature: Cannot set blood pool (Exc %d, pos [%d,%d,%d]).\n",
		...
		try{
			...
			error("TCreature::~TCreature: Cannot discard corpse/inventory (Exc %d, pos [%d,%d,%d], %s).\n",
	...
	for(TKnownCreature *KnownCreature = this->FirstKnowingConnection;
		...
		if(KnownCreature->CreatureID != this->ID){
			error("TCreature::~TCreature: Creature chaining error %u.\n", this->ID);
		}
...
int TCreature::GetHealth(void){
	...
	if(MaxHitPoints <= 0){
		if(!this->IsDead){
			error("TCreature::GetHealth: MaxHitpoints from %s is %d, even though she's not dead.\n",
			...
			if(ObjType.getFlag(PROTECTION) && ObjType.getFlag(CLOTHES)
				...
				if(ObjType.getFlag(WEAROUT)){
					...
					try{
						...
						error("TCreature::Damage: Exception %d when the object wears out %d.\n",
								r, ObjType.TypeID);
					}
				}
	...
	if(Damage == HitPoints){
		...
		if(Attacker == NULL){
			...
			if(DamageType == DAMAGE_PHYSICAL){
				...
				error("TCreature::Damage: Invalid damage type %d as a cause of death.\n", DamageType);
			}
		}else{
...
void InsertChainCreature(TCreature *Creature, int CoordX, int CoordY){
	if(Creature == NULL){
		...
		// until now.
		error("DeleteChainCreature: Deleted creature does not exist.\n");
		return;
	}
...
void ProcessCreatures(void){
	for(int Index = 0; Index < FirstFreeCreature; Index += 1){
		...
		if(!Creature->IsDead && Creature->Skills[SKILL_HITPOINTS]->Get() <= 0){
			error("ProcessCreatures: Creature %s is not dead even though she has no more HP.\n", Creature->Name);
			Creature->Death();
		}

		if(Creature->LoggingOut && Creature->LogoutPossible() == 0){ // LOGOUT_POSSIBLE ?
			if(Creature->IsDead && Creature->Skills[SKILL_HITPOINTS]->Get() > 0){
				error("ProcessCreatures: Creature %s has HP even though she is dead.\n", Creature->Name);
...
void LoadMonsterRaid(const char *FileName, int Start,
	...
	*Duration = 0;
	...
	if(Start >= 0){
		print(1, "Plan raid %s one for round %d.\n", FileName, Start);
	}
