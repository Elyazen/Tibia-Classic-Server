...
void SendAll(void){
	...
	while(Connection != NULL){
		if(Connection->WillingToSend){
			...
			error("SendAll: Connection is not ready to send.\n");
		}
...
bool BeginSendData(TConnection *Connection){
	...
	if(Connection != NULL && Connection->Live() && Connection->State != CONNECTION_LOGIN){
		...
		if(OutDataCommitted < OutDataCapacity){
			...
			print(2, "BeginSendData: Buffer from connection %d is full.\n", Connection->GetSocket());
		}
	}
	return Result;
}
...
void FinishSendData(TConnection *Connection){
	...
	if(Connection->Overflow){
		print(2, "FinishSendData: Buffer is full. Package will not be sent.\n");
		return;
	}
...
void SendDeleteInContainer(TConnection *Connection, int ContainerNr, Object Obj){
	...
	if(!Obj.exists()){
		error("SendDeleteInContainer: Object passed does not exist.\n");
		return;
	}
...
void SendPlayerData(TConnection *Connection){
	...
	TPlayer *Player = Connection->GetPlayer();
	if(Player == NULL){
		error("SendStatus: There is no player associated with this connection.\n");
		return;
	}
...
void SendTalk(TConnection *Connection, uint32 StatementID,
	...
	if(Sender == NULL){
		error("SendTalk (-): Transmitter is NULL.\n");
		return;
	}
...
void SendTalk(TConnection *Connection, uint32 StatementID,
	...
	if(Sender == NULL){
		error("SendTalk (C): Transmitter is NULL.\n");
		return;
	}
...
void SendTalk(TConnection *Connection, uint32 StatementID,
	...
	if(Sender == NULL){
		error("SendTalk (K): Transmitter is NULL.\n");
		return;
	}
