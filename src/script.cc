...
static void SaveFile(const char *Filename){
	...
	if(Dest == NULL){
		error("SaveFile: Can target file %s don't invest.\n", BackupFilename);
		fclose(Source);
		return;
	}
	...
	while(true){
		...
		if(fwrite(Buffer, 1, n, Dest) != n){
			error("SaveFile: Error writing target file %s.\n", BackupFilename);
			break;
		}
	}

	if(fclose(Source) != 0){
		error("SaveFile: Error %d when closing the source file.\n", errno);
	}

	if(fclose(Dest) != 0){
		error("SaveFile: Error %d when closing the target file.\n", errno);
	}
}
	...
	if(this->RecursionDepth != -1){
		...
		for(int Depth = this->RecursionDepth; Depth >= 0; Depth -= 1){
			...
			if(fclose(this->File[Depth]) != 0){
				::error("TReadScriptFile::close: Error %d when closing the file.\n", errno);
			}
		}
		this->RecursionDepth = -1;
	}
...
void TReadScriptFile::open(const char *FileName){
	...
	if(this->File[Depth] == NULL){
		int ErrCode = errno;
		::error("TReadScriptFile::open: Can file %s don't open.\n", this->Filename[Depth]);
...
void TReadScriptFile::nextToken(void){
	if(this->RecursionDepth == -1){
		::error("TReadScriptFile::nextToken: No script opened for reading.\n");
...
TWriteScriptFile::~TWriteScriptFile(void){
	if(this->File != NULL){
		...
		if(fclose(this->File) != 0){
			::error("TWriteScriptFile::~TWriteScriptFile: Error %d when closing the file.\n", errno);
		}
	}
}
...
void TWriteScriptFile::open(const char *FileName){
	if(this->File != NULL){
		...
		if(fclose(this->File) != 0){
			::error("TWriteScriptFile::open: Error %d when closing the file.\n", errno);
		}
		this->File = NULL;
	}
	...
	if(this->File == NULL){
		int ErrCode = errno;
		::error("TWriteScriptFile: Can file %s don't invest.\n", FileName);
		::error("Mistake %d: %s.\n", ErrCode, strerror(ErrCode));
...
void TWriteScriptFile::close(void){
	...
	if(fclose(this->File) != 0){
		::error("TWriteScriptFile::close: Error %d when closing the file.\n", errno);
	}
	this->File = NULL;
}
...
void TWriteScriptFile::error(const char *Text){
	if(this->File != NULL){
		if(fclose(this->File) != 0){
			::error("TWriteScriptFile::error: Error %d when closing the file.\n", errno);
		}
		this->File = NULL;
	}
...
void TReadBinaryFile::error(const char *Text){
	if(this->File != NULL){
		if(fclose(this->File) != 0){
			::error("TReadBinaryFile::error: Error %d when closing the file.\n", errno);
		}
		this->File = NULL;
	}
...
uint8 TReadBinaryFile::readByte(void){
	...
	if(Result != 1){
		...
		if(fclose(this->File) != 0){
			::error("TReadBinaryFile::readByte: Error %d when closing the file.\n", errno);
		}
		this->File = NULL;
...
void TReadBinaryFile::readBytes(uint8 *Buffer, int Count){
	...
	if(Result != Count){
		int ErrCode = errno;
		int Position = this->getPosition();
		::error("TReadBinaryFile::readBytes: Error reading from %d bytes\n", Count);
		::error("# File: %s, position %d, return value: %d, error code: %d (%s)\n",
				this->Filename, Position, Result, ErrCode, strerror(ErrCode));

		// NOTE(fusion): Close file and make a backup, possibly for further inspection.
		if(fclose(this->File) != 0){
			::error("TReadBinaryFile::readBytes: Error %d when closing the file.\n", errno);
		}
		this->File = NULL;
		SaveFile(this->Filename);
...
TReadBinaryFile::~TReadBinaryFile(void){
	if(this->File != NULL){
		...
		if(fclose(this->File) != 0){
			::error("TReadBinaryFile::~TReadBinaryFile: Error %d when closing the file.\n", errno);
		}
	}
}
...
void TWriteBinaryFile::open(const char *FileName){
	...
	if(this->File == NULL){
		int ErrCode = errno;
		::error("TWriteBinaryFile::open: Can file %s don't invest.\n", FileName);
		::error("Mistake %d: %s.\n", ErrCode, strerror(ErrCode));
...
void TWriteBinaryFile::error(const char *Text){
	if(this->File != NULL){
		if(fclose(this->File) != 0){
			::error("TWriteBinaryFile::error: Error %d when closing the file.\n", errno);
		}
		this->File = NULL;
	}
...
void TWriteBinaryFile::writeByte(uint8 Byte){
	...
	if(Result != 1){
		...
		// NOTE(fusion): Close file and make a backup, possibly for further inspection.
		if(fclose(this->File) != 0){
			::error("TWriteBinaryFile::writeByte: Error %d when closing the file.\n", errno);
		}
		this->File = NULL;
		SaveFile(this->Filename);
...
void TWriteBinaryFile::writeBytes(const uint8 *Buffer, int Count){
	...
	if(Result != Count){
		...
		// NOTE(fusion): Close file and make a backup, possibly for further inspection.
		if(fclose(this->File) != 0){
			::error("TWriteBinaryFile::writeBytes: Error %d when closing the file.\n", errno);
		}
		this->File = NULL;
		SaveFile(this->Filename);
...
TWriteBinaryFile::~TWriteBinaryFile(void){
	if(this->File != NULL){
		...
		if(fclose(this->File) != 0){
			::error("TWriteBinaryFile::~TWriteBinaryFile: Error %d when closing the file.\n", errno);
		}
	}
}
