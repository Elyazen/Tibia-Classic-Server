...
void GetCommunicationThreadStack(int *StackNumber, void **Stack){
	...
	*Stack = NULL;
	...
	if(!UseOwnStacks){
		error("GetCommunicationThreadStack: Library does not support its own stacks.\n");
		return;
	}
...
bool WriteToSocket(TConnection *Connection, uint8 *Buffer, int Size, int MaxSize){
	...
	while(BytesToWrite > 0){
		...
		if(BytesWritten > 0){
			...
			error("WriteToSocket: Error %d when sending to socket %d.\n",
...
bool SendLoginMessage(TConnection *Connection, int Type, const char *Message, int WaitingTime){
	...
	try{
		...
		error("SendLoginMessage: Error filling buffer (%s)\n", str);
		return false;
	}
}
...
void InsertWaitinglistEntry(const char *Name, uint32 NextTry, bool FreeAccount, bool Newbie){
	...
	if(NewEntry){
		Log("queue", "Add %s into the queue.\n", Name);
	}
}
...
int CheckWaitingTime(const char *Name, TConnection *Connection, bool FreeAccount, bool Newbie){
	...
	if((PlayersOnline + Position) > GetOrderBufferSpace()){
		...
		if(EarliestFreeAccountAdmissionRound > RoundNr){
			...
			print(3, "No more room for newbies with FreeAccount.\n");
...
TPlayerData *PerformRegistration(TConnection *Connection, char *PlayerName,
	...
	if(!QueryManagerConnection){
		error("PerformRegistration: Cannot connect to Query Manager.\n");
	...
	switch(LoginCode){
		...
		case 3:{
			print(3, "Player doesn't live in this world.\n");
			SendLoginMessage(Connection, LOGIN_MESSAGE_ERROR,
		...
		case 6:{
			Log("game", "Incorrect password for player %s; Login from %s.\n",
		...
		case 8:{
			Log("game", "Player's account %s was deleted.\n", PlayerName);
			SendLoginMessage(Connection, LOGIN_MESSAGE_ERROR,
					"Accountnumber or password is not correct.", -1);
			return NULL;
		}
		...
		case 9:{
			Log("game", "IP address %s for players %s blocked.\n",
		...
		case 15:{
			print(3, "Incorrect account number %u for %s.\n", AccountID, PlayerName);
			SendLoginMessage(Connection, LOGIN_MESSAGE_ERROR,
	...
	// `decrementIsOnline`.
	if(AccountID == 0){
		error("PerformRegistration: Player %s has not yet been assigned to an account.\n", PlayerName);
		SendLoginMessage(Connection, LOGIN_MESSAGE_ERROR,
...
bool HandleLogin(TConnection *Connection){
	...
	try{
		...
		error("HandleLogin: Error reading the command (%s).\n", str);
		return false;
	}
	...
	try{
		...
		if(!PrivateKey.decrypt(AsymmetricData) || AsymmetricData[0] != 0){
			RSAMutex.up();
			error("HandleLogin: Error decrypting.\n");
			SendLoginMessage(Connection, LOGIN_MESSAGE_ERROR,
					"Login failed due to corrupt data.", -1);
			return false;
		}
		...
		print(3, "Error reading the login data (%s).\n", str);
		SendLoginMessage(Connection, LOGIN_MESSAGE_ERROR,
				"Login failed due to corrupt data.",-1);
		return false;
	}
	...
	while(true){
		...
		if(!GetWaitinglistEntry(PlayerName, &NextTry, &FreeAccount, &Newbie)){
			print(3, "Players not on waiting list.\n");
			break;
		}

		print(3, "Players on waiting list.\n");
		if(NextTry > RoundNr){
			int WaitingTime = (int)(NextTry - RoundNr);
			Log("queue", "%s reports %d seconds too early.\n",
		...
		if(RoundNr > (NextTry + 60)){
			Log("queue", "%s reports %d seconds too late.\n",
	...
	if(!WasInWaitingList){
		...
		if(BlockLogin){
			...
			if(QueryManagerConnection){
				...
				error("HandleLogin: Cannot connect to Query Manager.\n");
			}
	...
	try{
		...
		error("HandleLogin: Error assembling login package (%s).\n", str);
		SendLoginMessage(Connection, LOGIN_MESSAGE_ERROR,
				"Internal error, closing connection.",-1);
		return false;
	}
...
bool ReceiveCommand(TConnection *Connection){
	...
	while(!Connection->WaitingForACK){
		...
		if(Size == 0 || Size > (int)sizeof(Connection->InData)){
			print(3, "packet to socket %d too big or empty, will be discarded (%d bytes)\n",
		...
		if(Connection->State == CONNECTION_CONNECTED){
			...
			if(PlainSize == 0 || (PlainSize + 2) > Size){
				print(3, "payload (%d Bytes) from packet to socket %d too big or empty.\n",
...
void CommunicationThread(int Socket){
	...
	if(Connection == NULL){
		...
		if(close(Socket) == -1){
			error("CommunicationThread: Error %d when closing the socket (1).\n", errno);
		}
		return;
	}
...

	{
		...
		if(fcntl(Socket, F_SETOWN_EX, &FOwnerEx) == -1){
			...
			if(close(Socket) == -1){
				error("CommunicationThread: Error %d when closing the socket (2).\n", errno);
			}
			Connection->Free();
			return;
		}
...

	{
		...
		if(SocketFlags == -1 || fcntl(Socket, F_SETFL, (SocketFlags | O_NONBLOCK | O_ASYNC)) == -1){
			...
			if(close(Socket) == -1){
				error("CommunicationThread: Error %d when closing the socket (3).\n", errno);
			}
			Connection->Free();
			return;
		}
	...
	// probably better to set it anyways to be sure.
	{
		...
		if(setsockopt(Socket, IPPROTO_TCP, TCP_NODELAY, &NoDelay, sizeof(NoDelay)) == -1){
			...
			if(close(Socket) == -1){
				error("CommunicationThread: Error %d when closing the socket (3.5).\n", errno);
			}
			Connection->Free();
			return;
		}
	...
	if(!Connection->SetLoginTimer(5)){
		...
		if(close(Socket) == -1){
			error("CommunicationThread: Error %d when closing the socket (4).\n", errno);
		}
		Connection->Free();
		return;
	}
...
bool OpenSocket(void){
	...
	if(listen(TCPSocket, 512) == -1){
		error("LaunchServer: Error %d at lists.\n", errno);
		return false;
	}
		...
		try{
			if(UseOwnStacks){
				...
				if(Stack == NULL){
					...
					if(close(Socket) == -1){
						error("AcceptorThreadLoop: Error %d when closing the socket (1).\n", errno);
					}
				}else{
					...
					if(ConnectionThread == INVALID_THREAD_HANDLE){
						...
						ReleaseCommunicationThreadStack(StackNumber);
						if(close(Socket) == -1){
							error("AcceptorThreadLoop: Error %d when closing the socket (2).\n", errno);
						}
					}
				}
			}else{
				if(ActiveConnections >= MAX_COMMUNICATION_THREADS){
					...
					if(close(Socket) == -1){
						error("AcceptorThreadLoop: Error %d when closing the socket (3).\n", errno);
					}
				}else{
					IncrementActiveConnections();
					void *Argument = (void*)((uintptr)Socket & 0xFFFF);
					...
					if(ConnectionThread == INVALID_THREAD_HANDLE){
						...
						DecrementActiveConnections();
						if(close(Socket) == -1){
							error("AcceptorThreadLoop: Error %d when closing the socket (4).\n", errno);
						}
					}
				}
			}
...
void ExitCommunication(void){
	...
	if(TCPSocket != -1){
		if(close(TCPSocket) == -1){
			error("ExitCommunication: Error %d when closing the socket.\n", errno);
		}
		TCPSocket = -1;
	}
